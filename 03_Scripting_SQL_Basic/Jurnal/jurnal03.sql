...
-- Query tambahan
-- Menampilkan semua teater dengan harga diskon
SELECT 
    ID_THEATER,
    KELAS,
    HARGA AS HARGA_ASLI,
...
-- menampilkan semua pemasukan masing masing teater
SELECT 
    th.ID_THEATER,
    SUM(th.HARGA) AS TOTAL_PEMASUKAN
FROM THEATER th
JOIN JADWAL jd ON th.ID_THEATER = jd.ID_THEATER
JOIN TRANSAKSI tr ON jd.ID_JADWAL = tr.ID_JADWAL
WHERE tr.STATUS = 'bayar'
